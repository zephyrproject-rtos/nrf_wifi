					       ...
					       unsigned int event_addr)
{
	...
	addr = (unsigned long)&hal_dev_ctx->rpu_info.soft_hpq->host_event_busy_index;
	status = hal_rpu_mem_read(hal_dev_ctx,
        ...
        if (status != NRF_WIFI_STATUS_SUCCESS) {
                nrf_wifi_osal_log_err("%s: Read from the event_busy_index = %x address failed, val (0x%X)\n",
                                       __func__,
				       (unsigned int)addr,
                                       current_index);
                goto out;
	...
	addr = (unsigned long)&hal_dev_ctx->rpu_info.soft_hpq->event_free_buffs[current_index];
	status = hal_rpu_mem_write(hal_dev_ctx,
				   (unsigned int)addr,
				   (void *)(unsigned long)event_addr,
				   sizeof(unsigned int));

	if (status != NRF_WIFI_STATUS_SUCCESS) {
		nrf_wifi_osal_log_err("%s: Writing to freebuffs[index=%d] address = %x failed for event_addr= %x\n",
				      __func__,
				      current_index,
				      (unsigned int)addr,
				      event_addr);
	...
	current_index++;
	if (current_index == NRF_WIFI_HOST_RPU_EVENT_BUFFERS)
		current_index = 0;
	...
	if (status != NRF_WIFI_STATUS_SUCCESS) {
		nrf_wifi_osal_log_err("%s: Writing to event_busy_index address = %x failed for val =%d\n",
				      __func__,
				      (unsigned int)addr,
				      current_index);
		goto out;
...
static unsigned int hal_rpu_event_get_all(struct nrf_wifi_hal_dev_ctx *hal_dev_ctx)
{
	...
	while (1) {
		...
		addr = (unsigned long)&hal_dev_ctx->rpu_info.soft_hpq->event_busy_buffs[cur_index];
		status = hal_rpu_mem_read(hal_dev_ctx,
					  &event_addr,
					  (unsigned int)addr,
					  sizeof(unsigned int));
		...
		if (status != NRF_WIFI_STATUS_SUCCESS) {
			nrf_wifi_osal_log_err("%s: Read from event_busy_buffs[%d]= %x addr failed, val (0x%X)\n",
					      __func__,
					      cur_index,
					      (unsigned int)addr,
					      event_addr);
		...
		if (!event_addr) {
			for (desc_no = 0; desc_no < NRF_WIFI_HOST_RPU_TX_DESC; desc_no++) {
				addr = (unsigned long)&hal_dev_ctx->rpu_info.soft_hpq->host_tx_done_busy_index;
				status = hal_rpu_mem_read(hal_dev_ctx,
							  &cur_index,
							  (unsigned int)addr,
							  sizeof(unsigned int));
				...
				if (status != NRF_WIFI_STATUS_SUCCESS) {
					nrf_wifi_osal_log_err("%s: Reading tx_done_busy_index = %x failed,val(0x%X)\n",
							      __func__,
							      (unsigned int)addr,
							      cur_index);
					goto out;
				...
				addr = (unsigned long)&hal_dev_ctx->rpu_info.soft_hpq->tx_done_buffs[cur_index];
				status = hal_rpu_mem_read(hal_dev_ctx,
							  &event_addr,
							  (unsigned int)addr,
							  sizeof(unsigned int));
				...
				if (status != NRF_WIFI_STATUS_SUCCESS) {
					nrf_wifi_osal_log_err("%s: Reading tx_done_buffs[%d] = %x failed, val (0x%X)\n",
							      __func__,
							      cur_index,
							      (unsigned char)addr,
							      event_addr);
				...
				if (!event_addr) {
					...
					if (status != NRF_WIFI_STATUS_SUCCESS) {
						nrf_wifi_osal_log_err("%s: Writing to tx_done_buffs[%d] = %x failed \n",
								      __func__,
								      cur_index,
								      (unsigned int)addr);
			                        goto out;
					...
					if (status != NRF_WIFI_STATUS_SUCCESS) {
						nrf_wifi_osal_log_err("%s: Writing %d to tx_done_busy_index %x failed \n",
								      __func__,
								      cur_index,
								      (unsigned int)addr);
			                        goto out;
			...
			if (status != NRF_WIFI_STATUS_SUCCESS) {
				nrf_wifi_osal_log_err("%s: Writing to event_busy_buffs[%d] = %x addr failed \n",
						      __func__,
						      cur_index,
						      (unsigned int)addr);
				goto out;
...
#else
		nrf_wifi_osal_log_dbg("Ignore watchdog interrupt, RPU recovery not needed");
