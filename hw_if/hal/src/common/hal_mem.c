					     ...
					     unsigned int len)
{
	 ...
	 * for RAM access
	 */
	status = pal_rpu_addr_offset_get(ROM_ACCESS_REG_ADDR,
	...
	if (len == 4) {
		*((unsigned int *)src_addr) = nrf_wifi_bal_read_word(
...
#ifdef WIFI_NRF71
static enum nrf_wifi_status rpu_mem_write_rom(
			...
			unsigned int len)
{
	...
	status = pal_rpu_addr_offset_get(rom_addr_val,
					 &addr_offset,
					 hal_dev_ctx->curr_proc);
...
#ifdef WIFI_NRF71
static enum nrf_wifi_status rpu_mem_write_code_ram(
			struct nrf_wifi_hal_dev_ctx *hal_dev_ctx,
			...
			unsigned int len)
{
	...
	status = pal_rpu_addr_offset_get(code_ram_addr_val,
					 &addr_offset,
					 hal_dev_ctx->curr_proc);
...
static enum nrf_wifi_status rpu_mem_write_data_ram(
			struct nrf_wifi_hal_dev_ctx *hal_dev_ctx,
			...
			unsigned int len)
{
	...
	status = pal_rpu_addr_offset_get(data_ram_addr_val,
					 &addr_offset,
					 hal_dev_ctx->curr_proc);
...
static enum nrf_wifi_status rpu_mem_read_code_ram(
			struct nrf_wifi_hal_dev_ctx *hal_dev_ctx,
			void *src_addr,
			unsigned int code_ram_addr_val,
			unsigned int len)
...
static enum nrf_wifi_status rpu_mem_read_data_ram(
			struct nrf_wifi_hal_dev_ctx *hal_dev_ctx,
			void *src_addr,
			unsigned int data_ram_addr_val,
			unsigned int len)
...
static enum nrf_wifi_status rpu_mem_write_secure_ram(
			struct nrf_wifi_hal_dev_ctx *hal_dev_ctx,
			...
			unsigned int len)
{
	...
	if ((secure_ram_addr_val < RPU_ADDR_SECURERAM_START) ||
		...
		nrf_wifi_osal_log_err("%s: Address not in range\n",
				      __func__);
		goto out;
	}
	for (i = 0; i < (len / sizeof(int)); i++) {
		...
		status = hal_rpu_reg_write(hal_dev_ctx,
					   ...
					   data);
		if (status != NRF_WIFI_STATUS_SUCCESS) {
			nrf_wifi_osal_log_err("%s: Writing to secure ram failed\n",
					       __func__);
			goto out;
		}
	}
				       ...
				       unsigned int len)
{
	...
	if (hal_rpu_is_mem_code_ram(rpu_mem_addr_val)) {
		...
		status = rpu_mem_write_secure_ram(hal_dev_ctx,
						  ...
						  src_addr,
						  len);
	} else if (hal_rpu_is_mem_ram(hal_dev_ctx->curr_proc,
				      rpu_mem_addr_val)) {
				     ...
				     enum HAL_RPU_MEM_TYPE mem_type)
{
	...
	if (mem_type == HAL_RPU_MEM_TYPE_ROM_0 ||
		...
		if (status != NRF_WIFI_STATUS_SUCCESS) {
			nrf_wifi_osal_log_err("%s: pal_rpu_addr_offset_get failed", __func__);
			goto out;
		}
